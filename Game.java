...
public class Game {
	public static void main(String[] args) {
        ...
        try {
			PrintWriter p1 = new PrintWriter(new FileWriter(new File("Player One")), true);
			try {
				PrintWriter p2 = new PrintWriter(new FileWriter(new File("Player Two")), true);
				while (wantToPlay) {
		            Deck deck = new Deck();
					int p = 0;
					int bet = 1;
					...
					p1.println("Player One has " + b1 + " chips remaining.\n");
					p1.println("Player One's hand:");
					for (int i = 0; i < 5; i++) {
						p1.println(h1[i]);
					}
					p1.println("\n");
					p2.println("Player Two has " + b2 + " chips remaining.\n");
					p2.println("Player Two's hand:");
					for (int i = 0; i < 5; i++) {
						p2.println(h2[i]);
					}
					p2.println("\n");
					...
					while (s1 == 1 && s2 == 1) {
						if (turn == 1) {
							...
							if (s1 >= 0) {
								...
								p += bet;
								minimum = bet;
								System.out.println("The pot contains " + p + " chips.");
								p1.println("Player One has " + b1 + " chips remaining.\n");
								p1.println("Player One's hand:");
								for (int i = 0; i < 5; i++) {
									p1.println(h1[i]);
